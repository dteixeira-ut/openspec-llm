...
1. **Select the change**
   ...
   Infer from context or prompt the user to choose from available changes.
   Announce: "Creating PR for change: <name>"

2. **Check pre-conditions**

   Confirm a review has been run by looking for recent `APPROVED` output in the conversation. If not found, warn and ask: "Proceed without review?" Stop if no.
...
9. **Poll for reviewer response**

   Record baseline comment count, then check every 2 minutes up to 5 times (10 min max):
   - If a configured reviewer comments → notify user with preview, stop
   - After 5 checks with no response → alert: "No AI reviewer response after 10 minutes. Check: <URL>"
