...
- **Rule names use kebab-case** (e.g. `error-shape`, `audience-prefix`) rather than CamelCase or snake_case. Alternative — match the Confluence page's title casing — rejected because rule names are referenced from other SKILL.md files (links, anchors); kebab-case is the markdown-idiomatic anchor format and matches the existing skill-naming convention.
- **The standards are SNAPSHOTTED into the skill, not fetched live from Confluence.** Alternative — make the skill a fetcher that pulls the current Confluence content at invocation time — rejected because: (a) Confluence requires authentication, raising the bar for cross-team adoption; (b) the rules change rarely (Oct 2025 was the last update); (c) the staleness risk is mitigated by the graduation criteria (when UT moves to a programmatic reference, the skill follows). Snapshot freshness is the trade-off; the alternative was deemed costlier.
- **Section per Confluence page** (Framework / REST API / Languages) rather than section per rule type (URI / Response / Auth). Alternative — group by rule type to make cross-page rule comparison easier — rejected because Confluence pages are the canonical source of truth; mirroring their structure keeps the skill auditable against the source.
- **No code samples in the standards skill body.** Alternative — include sample TypeScript snippets for each rule — rejected because (a) the rule should be language-agnostic at the standards layer; (b) NestJS-specific implementation examples belong in `migrate-to-nestjs/SKILL.md` where they have surrounding context; (c) sample code in a standards body drifts faster than prose rules.
